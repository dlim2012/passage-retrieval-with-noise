...
def parse():
    ...
    parser.add_argument('--train_data_file', type=str,
                        default='/mnt/ssd/data/ms_marco/preprocessed/triples.train.small.shuffled.tsv')
...
n_max_validation_instances = 5000
...
train_data_file = '/mnt/ssd/data/ms_marco/preprocessed/train.tsv'
validation_data_file = '/mnt/ssd/data/ms_marco/preprocessed/validation.tsv'
...
def main():
    ...
    with open(train_data_file, 'r') as f:
        for i, line in enumerate(f):
            line = line.split('\t')
            if len(train_data) == n_train_instances:
                break
            if random.random() < 1 / args.npr:
                train_data.append([line[0], line[1], 1])
            if len(train_data) == n_train_instances:
                break
            train_data.append([line[0], line[2], 0])

    validation_data = []
    with open(validation_data, 'r') as f:
        for i, line in enumerate(f):
            line = line.split('\t')
            if len(validation_data) == n_max_validation_instances:
                break
            if random.random() < 1 / args.npr:
                validation_data.append([line[0], line[1], 1])
            if len(validation_data) == n_max_validation_instances:
                break
            validation_data.append([line[0], line[2], 0])
    ...
    for epoch in range(1):
        ...
        for i, data in pbar:
            ...
            if (i + 1) % args.measure_steps == 0:
                ...
                if avg_loss > sum([info[1] for info in ckpt_info]) / len(ckpt_info) * 1.1:
                    early_stop_count += 1
            ...
            if (i + 1) % validation_step == 0:
                ...
                for j, val_data in enumerate(validation_dataloader):
                    ...
                    running_loss += loss.item()
                    running_accuracy += torch.sum(
                        (torch.argmax(outputs.logits, dim=1) == val_data['labels']).type(torch.int32)).item() / len(
                        val_data['labels'])
